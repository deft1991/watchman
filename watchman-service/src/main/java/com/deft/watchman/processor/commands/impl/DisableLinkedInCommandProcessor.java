...
public class DisableLinkedInCommandProcessor extends BasicCommandProcessor {
    ...
    public void processCommand(AbilityBot bot, Update update, ChatSettings chatSettings) {
        try {
            ...
            if (bot.isGroupAdmin(chat.getId(), from.getId())) {
                chatSettings.setLinkedinEnable(false);
                chatSettingsRepository.save(chatSettings);
            }
        } catch (Exception ex) {
            log.warn("Err in DisableLinkedInCommandProcessor ex: {}", ex.getMessage());
        }
    }

    @Override
    public CommandType getProcessorType() {
        return CommandType.DISABLE_LINKEDIN;
    }
}
