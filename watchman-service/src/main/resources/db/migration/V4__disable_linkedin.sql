UPDATE public.message_dictionary
SET
    update_date = now(),
    message     = 'Привет %s (@%s), представься пожалуйста с тегом #whois

В приветствии напиши - как нашел чат, какие у тебя планы на собесы, куда готовишься? Расскажи про  свой опыт и ссылку на линкадин. Чем интересней о себе расскажешь тем полезней будет для тебя этот чат! 🤓

Осторожно! В чате работает строгий вахтер, который выкидывает тех, кто не написал сообщение #whois и не прислал ссылку на линкедИн'
WHERE type = 'JOIN_GROUP_MESSAGE';

INSERT INTO message_dictionary (id, type, message)
VALUES (gen_random_uuid(),
        'JOIN_GROUP_MESSAGE_WITHOUT_LINKEDIN',
        'Привет %s (@%s), представься пожалуйста с тегом #whois

В приветствии напиши - как нашел чат, какие у тебя планы на собесы, куда готовишься? Расскажи про  свой опыт. Чем интересней о себе расскажешь тем полезней будет для тебя этот чат! 🤓

Осторожно! В чате работает строгий вахтер, который выкидывает тех, кто не написал сообщение #whois');

UPDATE public.message_dictionary
SET
    update_date = now(),
    message     = 'Добро пожаловать, %s (@%s)!'
WHERE type = 'WELCOME_MESSAGE';
